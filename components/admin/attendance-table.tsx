...
export function AttendanceTable({ selectedDate }: AttendanceTableProps) {
  ...
  const departments = Array.from(new Set(attendanceData.map(record => record.department)))
  ...
  const handleTimeIn = (employeeId: string) => {
    ...
    // Refresh data
    const records = dataStore.getAttendanceRecords(selectedDate || format(new Date(), "yyyy-MM-dd"))
    const employees = dataStore.getEmployees()
    const combinedData = employees.map(employee => {
      const record = records.find(r => r.employeeId === employee.employeeId)
      return {
                ...
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm text-gray-400">Add employees to start tracking attendance</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
